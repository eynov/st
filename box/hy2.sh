...
# 1. 动态安装官方最新版核心 (含高容错 API 校验)
install_sing_box_core() {
    if [ -f "$BINARY_PATH" ]; then
        SB_VER=$($BINARY_PATH version | awk '{print $3}')
        echo "🟢 sing-box 已安装，当前版本: $SB_VER"
        return
    fi
    
    echo "⚠️ 推荐使用 sing-box ≥ 1.9.0 以获得完美的 Hy2 体验"
