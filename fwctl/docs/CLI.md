...
安装后命令名为 `fw`。本文档是 v4 的完整命令契约：v3 已有的命令必须逐字保持
行为不变，新增命令遵循统一的名词 + 动词结构。

## 兼容承诺

以下入口在 v4 中行为不变，包括输出文本、退出码和交互流程：
...
`port` 是 v3 遗留形态，只支持 `add`、`remove`、`list`（`remove` 是 v3 拼写，
保留；同时接受 `delete` 作为别名）。
...
|---|---|
| 0 | 成功 |
| 1 | 运行时失败（渲染、apply、系统调用） |
| 2 | 用法错误（未知命令、参数个数不对） |
| 3 | 校验失败（schema、语义、引用不存在） |
| 4 | 获取全局写锁失败（另一个 fwctl 事务进行中） |
| 5 | 事务已回滚（apply 后验证失败，系统已恢复到变更前状态） |

退出码 5 与 1 的区别很重要：5 明确表示**内核状态已经回滚**，运维不需要手工
恢复；1 表示失败发生在 apply 之前，系统本就未被改动。
...
语义与 v3 一致：`both` 是一次逻辑操作，在 tcp 与 udp 各写一份；重复添加幂等
并提示；删除不存在的端口提示且不改状态;非法协议与非法端口返回非零且不改状态。

## target

```bash
fw target add <name> <address>[,<address>...]
...
fw target edit <name|id> [--name NEW] [--address LIST] [--add-address A] [--del-address A]
...
删除被规则引用的 target 返回退出码 3 并列出引用方；`--cascade` 连同引用它的
规则一起删除，需要 `--yes` 或交互确认。

## service

```bash
fw service add <name> <protocol> <port>[,<port>...]
fw service add https both 443
fw service add hy2-hop udp 60000-61000

fw service edit <name|id> [--name NEW] [--protocol P] [--ports LIST] [--add-port P] [--del-port P]
fw service delete <name|id> [--cascade]
fw service list [--json]
fw service show <name|id> [--json]
fw service enable <name|id>
fw service disable <name|id>
```

## rule

```bash
fw rule add <name> --type accept --service <ref> [--priority N] [--comment TEXT]
...
fw rule edit <name|id> [--name NEW] [--service R] [--target R] [--source R]
                       [--to-port P|--no-translate] [--priority N] [--comment TEXT]
...
5. 迁移：是否存在待迁移的 v1 状态。
6. 转发：`net.ipv4.ip_forward` 与是否存在 forward 规则相符。
7. SSH 自锁风险：探测到的 SSH 端口是否会被当前策略放行。
8. 遗留表：是否仍存在 `sb_filter` / `sb_nat`。
9. 漂移：运行中的 `table ip fwctl` 是否与当前 state 渲染结果一致。
10. 持久化：`/etc/nftables.conf` 是否与渲染结果一致，`nftables.service` 是否
    enabled。

FAIL 时返回退出码 1，仅 WARN 返回 0。
...
`fw` 不带参数时进入菜单。v3 的 1–12 项保持不变，新增：
...
菜单项 10、11 继续输出「防护逻辑内嵌于模板」的提示，但 v4 中改为指向
`settings.policy`——提示文本更新，编号与位置不变。
