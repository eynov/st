...
REAL_CORE="${SB_TEST_REAL_CORE:-/tmp/sb-core-1.14.0/sing-box}"
[[ -x "$REAL_CORE" ]] || {
...
new_env() {
    ...
    export SB_CORE_ARCHIVE="/tmp/sb-core-1.14.0/sing-box.tar.gz"
...
# Build a self-contained previous-pin manager/core fixture.  The fake 1.13.21
...
make_previous_pin_source() {
    ...
    binary="$payload/sing-box-1.13.21-linux-amd64/sing-box"
    mkdir -p "$(dirname "$binary")" || return 1
    printf '%s\n' \
      '#!/usr/bin/env bash' \
      'if [[ "${1:-}" == version ]]; then' \
      '  printf "sing-box version 1.13.21\\n"' \
      'else' \
      "  exec \"$REAL_CORE\" \"\$@\"" \
      'fi' >"$binary" || return 1
    chmod 755 "$binary" || return 1
    tar -czf "$archive" -C "$payload" sing-box-1.13.21-linux-amd64 || return 1
    ...
    jq -n --arg archive_sha "$archive_sha" --arg binary_sha "$binary_sha" '{
      schema_version:1,
      source:"https://api.github.com/repos/SagerNet/sing-box/releases/tags/v1.13.21",
      versions:{"1.13.21":{"linux-amd64":{
...
setup_previous_pin_install() {
    ...
    old_archive="$TEST_ROOT/sing-box-1.13.21.tar.gz"
...
test_cross_pin_upgrade_bootstrap() {
    ...
    assert "cross-pin refusal names the old and new pins" sh -c \
      "printf '%s' \"\$1\" | rg -q '1.13.21 -> 1.14.0'" _ "$out"
...
test_root_installer_and_core_archive() {
    ...
    new_env
    root="$TEST_ROOT"
    archive="/tmp/sb-core-1.14.0/sing-box.tar.gz"
    ...
    assert "pinned archive installs expected core" test \
      "$("$SB_BIN" version | awk 'NR==1{print $3}')" = "1.14.0"
    rm -f "$SB_BIN"
...
test_core_digest_adversarial() {
    ...
    init_env
    printf '%s\n' '#!/usr/bin/env bash' \
      'if [[ "${1:-}" == version ]]; then echo "sing-box version 1.14.0"; else exit 0; fi' \
      >"$root/bin/sing-box"
    chmod 755 "$root/bin/sing-box"
    if sb doctor --json >/dev/null 2>&1; then
    ...
    assert "ordinary node operation never replaces the core" rg -q \
      'sing-box version 1.14.0' "$root/bin/sing-box"
    sb core install >/dev/null
    ...
    jq '.versions["1.14.0"]["linux-amd64"].sha256="aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"' \
